...
export class CreateProjectDto {
  ...
  @IsUrl(
    ...
    { message: 'Old Site URL must be a valid HTTP/HTTPS URL' },
  )
  ...
  @IsUrl(
    { require_protocol: true, protocols: ['http', 'https'] },
    { message: 'New Site URL must be a valid HTTP/HTTPS URL' },
  )
  @Transform(({ value }) => sanitizeHtml.default(value?.trim()))
  newSiteUrl: string;
  ...
  @IsUrl(
    { require_protocol: true, protocols: ['http', 'https'] },
    { message: 'Sitemap URL must be a valid HTTP/HTTPS URL' },
  )
  sitemapUrl?: string;
}
